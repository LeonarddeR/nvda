...
def isForcedWordSeparator(ch: str) -> bool:
	"""Whether a character always ends a typed word, regardless of how the application groups it
	into a word unit.

	Characters with a Letter, Mark or Number Unicode category and apostrophes are word characters.

	:param ch: The character to classify.
	:return: ``True`` if the character forces a word boundary, ``False`` otherwise.
	"""
	return ch not in "'’" and unicodedata.category(ch)[0] not in "LMN"


def getLastTypedWord(text: str) -> str:
	"""Returns the last run of word characters in ``text``, ignoring trailing forced separators.

	:param text: The application's word-unit text ending at the caret.
	:return: The typed word, e.g. ``bar`` for ``foo.bar.``; empty when ``text`` contains no word characters.
	"""
	end = len(text)
	while end > 0 and isForcedWordSeparator(text[end - 1]):
		end -= 1
	start = end
	while start > 0 and not isForcedWordSeparator(text[start - 1]):
		start -= 1
