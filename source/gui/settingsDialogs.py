...
class AdvancedPanelControls(
	...
	def __init__(self, parent):
		...
		# Translators: This is the label for a combo-box control in the
		#  Advanced settings panel.
		label = _("Use the modern regular expression engine for speech dictionary entries:")
		self.useModernRegexEngineCombo: nvdaControls.FeatureFlagCombo = speechGroup.addLabeledControl(
			labelText=label,
			wxCtrlClass=nvdaControls.FeatureFlagCombo,
			keyPath=["featureFlag", "useModernRegexEngine"],
			conf=config.conf,
		)
		self.bindHelpEvent("UseModernRegexEngine", self.useModernRegexEngineCombo)
	...
	def haveConfigDefaultsBeenRestored(self):
		return (
			...
			and self.useModernRegexEngineCombo.isValueConfigSpecDefault()
	...
	def restoreToDefaults(self):
		...
		self.useModernRegexEngineCombo.resetToConfigSpecDefault()
	...
	def onSave(self):
		...
		self.useModernRegexEngineCombo.saveCurrentValueToConf()
